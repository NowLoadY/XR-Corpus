...
数据库使用 SQLite `user_version=4` 和 DELETE journal。退出 XRTranslate 后，可以直接复制
...
节点必须归属一个领域。领域可逐级嵌套，例如“Games → Overwatch → Heroes”；关闭任一
上级领域会暂停其下所有词汇与触发关系，子级原有开关状态保持不变。关闭节点或关系只停止
该节点或关系。

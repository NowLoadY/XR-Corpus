...
- `Triggers` 用于根据最近成功转写和未来实时 hints 激活 corpus；规范 Trigger 可以表达标题等稳定双语映射。
...
- `Terms` 是可按当前翻译任务语言选择并注入 ASR 的术语。
- 空行和以 `>` 开头的说明行不作为数据；其它未定义二级节只供维护者阅读。
- `id`、`domain`、`subdomain` 由规范目录路径推导，不在文件内重复维护：
  `<domain>.<subdomain>.<corpus-file-stem>`。
